...
const lightTools = [
  { type: "light-red", label: "Red", color: "#ff4d63", limit: 1 },
  { type: "light-blue", label: "Blue", color: "#5ca7ff", limit: 1 },
  { type: "light-yellow", label: "Yellow", color: "#ffdf5a", limit: 1 },
  { type: "light-green", label: "Green", color: "#63c96b", limit: 1 },
  { type: "light-purple", label: "Purple", color: "#bd78ff", limit: 1 }
];
...
const defaultPositions = {
  yafania: [{ x: 50, y: 53 }],
  mic: [{ x: 50, y: 43 }],
  "speaker-left": [{ x: 20, y: 70 }],
  "speaker-right": [{ x: 80, y: 70 }],
  "speaker-center": [{ x: 50, y: 73 }],
  drummer: [{ x: 50, y: 28 }],
  bassist: [{ x: 35, y: 50 }],
  keys: [{ x: 65, y: 50 }],
  guitarist: [{ x: 75, y: 58 }],
  ...
  "light-red": [{ x: 25, y: 22 }],
  "light-blue": [{ x: 35, y: 22 }],
  "light-yellow": [{ x: 50, y: 20 }],
  "light-green": [{ x: 65, y: 22 }],
  "light-purple": [{ x: 75, y: 22 }]
};
...
function renderPanel() {
  const builders = {
    Venue: venuePanel,
    Stage: () => toolPanel("Choose stage pieces to add. Tap the stage to reposition the selected piece.", stageTools),
    Band: () => toolPanel("Add each musician once. Tap the stage to reposition the selected musician.", bandTools),
    Dancers: dancerPanel,
    Lights: lightPanel,
...
function outfitPanel() {
  ...
  wrap.append(yafaniaMovePanel());
  return wrap;
}

function yafaniaMovePanel() {
  const wrap = document.createElement("div");
  wrap.className = "move-panel";
  wrap.append(meta("Move Yafania on stage."));
  ...
  [
    ...
    const button = document.createElement("button");
    button.type = "button";
    button.className = `move-button ${control.direction}`;
    button.textContent = control.label;
    button.addEventListener("click", () => moveYafania(control.direction));
...
function placeItem(event) {
  ...
  addItem(state.activeTool, x, y);
}

function addItemFromPanel(type) {
  const position = nextDefaultPosition(type);
  addItem(type, position.x, position.y);
}

function addItem(type, x, y) {
  const tool = [...stageTools, ...bandTools, ...lightTools].find((entry) => entry.type === type);
  state.activeTool = type;

  if (type === "dancer" && countItems("dancer") >= 5) return;
  if (type.startsWith("light") && countLights() >= 5 && countItems(type) === 0) return;
  if (tool && tool.limit && countItems(type) >= tool.limit) {
...
function moveYafania(direction) {
  let yafania = state.items.find((item) => item.type === "yafania");
  if (!yafania) {
    addItem("yafania", 50, 53);
    yafania = state.items.find((item) => item.type === "yafania");
  }
  ...
  yafania.x = Math.max(12, Math.min(88, yafania.x + move.x));
  yafania.y = Math.max(22, Math.min(78, yafania.y + move.y));
  state.activeTool = "yafania";
  render();
}

function nextDefaultPosition(type) {
  const positions = defaultPositions[type] || [{ x: 50, y: 50 }];
  const index = type === "dancer" ? countItems("dancer") : countItems(type);
